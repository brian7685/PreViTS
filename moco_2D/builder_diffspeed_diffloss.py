...
class Loss(nn.Module):
    ...
    def forward(
        ...
        ce1 = self._cross_entropy_loss(output, target)
        
        return ce1#, ce1 + ce2, ranking


class MoCoDiffLossTwoFc_standard(nn.Module):
    def __init__(
            self,
            base_encoder,
            dim=128,
        ...
        if mlp:  # hack: brute-force replacement
            dim_mlp = self.encoder_q.fc1.weight.shape[1]
            self.encoder_q.fc1 = nn.Sequential(nn.Linear(dim_mlp, dim_mlp), nn.ReLU(), self.encoder_q.fc1)
            self.encoder_k.fc1 = nn.Sequential(nn.Linear(dim_mlp, dim_mlp), nn.ReLU(), self.encoder_k.fc1)
    ...
    def _forward_encoder_k(self, im_k: Tensor):
        ...
        im_k, idx_unshuffle = self._batch_shuffle_ddp(im_k)
        #change here
        #k_A, k_M, feature = self.encoder_k(im_k)  # keys: NxC
        k_A, k_M = self.encoder_k(im_k)
        # undo shuffle
    ...
    def _diff_speed(self, im_q: Tensor, im_k: Tensor):
        B, C, T, H, W = im_q.shape
        random_indices = torch.randperm(B, device=im_q.device) #B= 16
        #random_indices = torch.arange(0, B, 1, device=im_q.device) #inorder
        selected_s1 = random_indices[:int(B * self.alpha)] #first 8 batch
        selected_s2 = random_indices[int(B * self.alpha):] #last 8 batch 

        diff_speed = random.choice(self.diff_speed) #2
        T_real = T // diff_speed #T=32 T_real =16
        speed1 = torch.arange(0, T, 1, device=im_q.device)[: T_real]             # 0-15 speed1 is normal speed
        speed2 = torch.arange(0, T, diff_speed, device=im_q.device)[: T_real]   # 0,2,..32 speed2 is randomly selected from self.diff_speed
        im_q_real = torch.empty(B, C, T_real, H, W, device=im_q.device)
        im_k_real = torch.empty_like(im_q_real)
        im_k_negative = torch.empty_like(im_q_real)
        #first half batch normal with 16 frames, second half batch with twice speed
        im_q_real[selected_s1] = im_q.index_select(0, selected_s1).index_select(2, speed1) #first half batch, second select time, cut time in half
        im_q_real[selected_s2] = im_q.index_select(0, selected_s2).index_select(2, speed2) #half with normal speed half with double speed
    ...
    def forward(self, im_q, im_k, add_to_queue= True, return_intermediate_outputs = False):
        ...
        for idx, (name, layer) in enumerate(self.encoder_q.encoder.named_children()):
            #print('name',name)
            #print('layer',layer)
            
            #im_q_0 = im_q[:,:,0,:,:]
            #print('im_q shape after',im_q.shape)
            if name in ['fc']:
              #print('fc',name)
              break
            out = layer(im_q) if idx == 0 else layer(out)
            ...
            if name in ['layer4']:
                intermediate_outputs[name] = out
        #"""  
        #print('intermediate_outputs',intermediate_outputs['layer4'].shape)
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            #if self.diff_speed is not None:
            #    im_q, im_k, k_neg_A, k_neg_M = self._diff_speed(im_q, im_k)  # Update im_q, im_k, only select first half of frames
            #print('im_q',im_q.shape)
            #im_q_real = im_q[:,:,:16,:,:]
            #print('im_q_real',im_q_real.shape)
            #im_k_real = im_k[:,:,:16,:,:]
            #im_q = im_q_real.clone()
            #im_k = im_k_real.clone()
            k_A, k_M = self._forward_encoder_k(im_k)

        # compute query features
        #if return_intermediate_outputs==True:
        #    q_A, q_M, intermediate_outputs['layer4'] = self.encoder_q(im_q)  # queries: NxC
        #else:
        #    q_A, q_M, _ = self.encoder_q(im_q)
        #q_A, q_M, intermediate_outputs['layer4'] = self.encoder_q(im_q)  # queries: NxC
        ...
        if return_intermediate_outputs:
            #return logits_A, labels_A, logits_M, labels_M, intermediate_outputs
            return logits_A, labels_A, 0, labels_M, intermediate_outputs
        #return logits_A, labels_A, logits_M, labels_M
        return logits_A, labels_A, 0, labels_M
...
class MoCoDiffLossTwoFc(nn.Module):
    ...
    def _forward_encoder_k(self, im_k: Tensor):
        ...
        im_k, idx_unshuffle = self._batch_shuffle_ddp(im_k)
        #change here
        k_A, feature = self.encoder_k(im_k)  # keys: NxC
        #k_A, k_M = self.encoder_k(im_k)
        # undo shuffle
        k_A = self._batch_unshuffle_ddp(k_A, idx_unshuffle)
        #k_M = self._batch_unshuffle_ddp(k_M, idx_unshuffle)
    ...
    def forward(self, im_q, im_k, add_to_queue= True, return_intermediate_outputs = False):
        ...
        with torch.no_grad():  # no gradient to keys
            self._momentum_update_key_encoder()  # update the key encoder
            B, C, T, H, W = im_q.shape
            im_q_real = torch.empty(B, C, T, H, W, device=im_q.device)
            im_k_real = torch.empty_like(im_q_real)
            im_k_real[:] = im_k[:]
            #im_k_negative = torch.empty_like(im_q_real)
            # if self.diff_speed is not None:
            ...
            #im_q = im_q_real.clone()
            #im_k = im_k_real.clone()
            #print('im_k',im_k.shape) #[8, 3, 1, 224, 224
            k_A = self._forward_encoder_k(im_k[:,:,0,:,:])
        ...
        #else:
        #    q_A, q_M, _ = self.encoder_q(im_q)
        q_A, intermediate_outputs['layer4'] = self.encoder_q(im_q[:,:,0,:,:])  # queries: NxC
        #q_A, q_M = self.encoder_q(im_q)
        #self.encoder_q(im_q)
        # compute logits
        # Einstein sum is more intuitive
        ...
        # logits: Nx(1+K)
        logits1 = torch.cat([l_pos_A1, l_neg_A], dim=1)
        #logits2 = torch.cat([l_pos_A1, l_neg_A], dim=1)
        #logits_A = (logits1, logits1)
        #logits_M = (l_pos_M, l_neg_M)  # l_pos > l_neg_speed
        ...
        if return_intermediate_outputs:
            #return logits_A, labels_A, logits_M, labels_M, intermediate_outputs
            return logits1, labels_A, intermediate_outputs
        #return logits_A, labels_A, logits_M, labels_M
        return logits1, labels_A