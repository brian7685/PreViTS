...
class S3D_G(nn.Module):
    def __init__(self, num_classes=400, drop_prob=0.5, in_channel=3, gate=True):
        ...
        self.feature = nn.Sequential(OrderedDict([
            ...
            ('maxpool4', nn.MaxPool3d(kernel_size=(3,2,2),stride=(2,2,2),padding=(0,0,0))),              # (832,8,7,7)
            ...
            # ('avgPool', nn.AvgPool3d(kernel_size=(2, 7, 7), stride=1)),                                 # (1024,7,1,1)
            # ('avgPool', nn.AdaptiveAvgPool3d((1, 1, 1))),  # 忽略输入大小
            # ('drop', nn.Dropout3d(drop_prob)),
            # ('conv3d', nn.Conv3d(1024, num_class, kernel_size=1, stride=1, bias=True)),                # (num_class,7,1,1)
        ]))
...
if __name__ == "__main__":
    ...
    # p, out = model(x)
    out = model.get_feature(x)
    print('get_feature',out.shape)
    out = model(x)
    print('model',out.shape)
#get_feature torch.Size([1, 1024, 2, 7, 7])
#avg shape torch.Size([1, 1024, 1, 1, 1])