...
class _BaseWrapper(object):
    ...
    def _encode_one_hot(self, ids):
        ...
        one_hot = torch.zeros_like(self.logits2).to(self.device)
        one_hot[:,0]=1
        return one_hot
    ...
    def forward(self, image_q, image_k, add_to_queue):
        ...
        self.logits2, self.target2,  self.intermediate= self.model(image_q, image_k, add_to_queue, return_intermediate_outputs=True)
        #self.logits2, self.target2 ,_,_2= self.model(image_q, image_k, add_to_queue)
        #self.logits = self.logits2[0]
        return self.logits2, self.target2
...
class GradCAM(_BaseWrapper):
    ...
    def backward(self, ids, target_layer):
        ...
        grad_wrt_act = torch.autograd.grad(outputs=self.logits2, inputs=fmaps, \
        grad_outputs=one_hot, create_graph=True)[0]
