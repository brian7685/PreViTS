...
class ResNet(nn.Module):
    ...
    def get_feature(self, x):
        ...
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x) #([1, 64, 16, 112, 112])
        #print('before max',x.shape)
        x = self.maxpool(x) #([1, 256, 8, 56, 56])
        #print('after max',x.shape)

        x = self.layer1(x) #([1, 256, 8, 56, 56])
        #print('after layer1',x.shape)

        x = self.layer2(x) #([1, 512, 4, 28, 28])
        #print('after layer2',x.shape)

        x = self.layer3(x) #([1, 1024, 2, 14, 14])
        #print('after layer3',x.shape)

        feat = self.layer4(x) #([1, 2048, 1, 7, 7])
        #print('after layer4',x.shape)

        return feat

    def get_output_and_feature(self, x):
