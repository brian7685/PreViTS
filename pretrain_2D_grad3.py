...
from moco_2D import ModelFactory
from moco_2D.builder_diffspeed_diffloss import Loss
from utils.moco import replace_moco_k_in_config
from grad_cam_2D import GradCAM
import cv2
...
class Engine:
    ...
    def __init__(self, args: Args, cfg: ConfigTree, local_rank: int):
        ...
        self.learning_rate = float(self.args.lr)#self.cfg.get_float('optimizer.lr')
        ...
        if int(self.args.adamw)==1:
            ...
            self.optimizer = optimizer.LARS(self.model.parameters(), self.learning_rate,
                                        weight_decay=1e-6,
                                        momentum=0.9)
        elif int(self.args.lamb)==1:
            self.optimizer = optim.Lamb(
                self.model.parameters(),
                lr= self.learning_rate,
                betas=(0.9, 0.999),
                eps=1e-8,
    ...
    def train_epoch(self,gcam_q):
        ...
        #for i, ((clip_q, clip_k),  *_) in enumerate(iter_data):
            ...
            output, target = gcam_q.forward(clip_q, clip_k, add_to_queue=True)
            # start from RSP on VGG sound training
            #loss_s = self.criterion(output, target)
            _cross_entropy_loss = nn.CrossEntropyLoss().to(self.device)
            loss_s = _cross_entropy_loss(output, target)
            
            
            mocoW = int(self.args.mocoW)
            loss_s = loss_s*mocoW
            loss_A = loss_s
            ...
            if grad_loss==1:# and load_mask==1:
                ...
                #print('mask_k',mask_k)
                #print('info',info)
                #mask_k = mask_k.repeat(1,1,16,1,1)
                ...
                output_masked, target_masked = gcam_q.forward(clip_q, masked_key, add_to_queue=False)
                # compute gradients of the dot-product wrt query convolutional activations
                #"""
                grad_wrt_act_masked = gcam_q.backward( ##[16, 2048, 1, 7, 7]
                    torch.zeros(0, dtype=clip_q.dtype, device=self.device),
                    target_layer=target_layer_q
                )
                #print('grad_wrt_act_masked',grad_wrt_act_masked.shape)
                # combine gradients with forward query activation maps to get gradcam
                gcam_masked = gcam_q.generate( ##[16, 1, 1, 7, 7]
                ...
                #print('mask_query_interp',mask_query_interp_2d.shape)
                ...
                loss= loss_s +loss_gcam_masked_total*gw
                
                loss_M = gw*loss_gcam_masked_total
                #print('gcam_masked',gcam_masked.shape)
                #with weight
                #loss= loss*0.25+loss_gcam_masked_total*0.75
                #only with mask
                #
            else:
                loss = loss_s
            ...
            acc1_A, acc5_A = accuracy(output, target, topk=(1, 5))
            #acc1_M, = accuracy(torch.cat(ranking_logits, dim=1), target, topk=(1,))

            #acc1_A_n, acc5_A_n = accuracy(output, target, topk=(1, 5))
