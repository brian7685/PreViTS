...
def get_random_crop_coords_mask_cover_center(
    img: np.ndarray,
    reference_mask: np.ndarray,
    other_coords: Union[Tuple, List[Tuple]] = [],
    min_areacover: float = 0.2,
    ...
    in_ratio = float(image_width) / float(image_height)
    #print('start center')
    if (in_ratio < 0.75):
        w = image_width
        h = int(round(w / 0.75))
    ...
    j = (image_width - w) // 2
    #print('end center')
    return (j, i, j+w, i+h), 1
    

def get_random_crop_coords_mask_cover_one(
    img: np.ndarray,
    reference_mask: np.ndarray,
    other_coords: Union[Tuple, List[Tuple]] = [],
    min_areacover: float = 0.2,
...
def get_reference_crop_covering_mask(
    ...
    if np.amax(img_or_reference_mask) == 1:
        ...
        #mask = np.ones_like(img_or_reference_mask, (height, width), np.uint8)
        mask = np.ones_like(img_or_reference_mask, dtype=np.uint8)
    ...
    mask = np.array(crop(mask, *reference_coords))
...
def random_con_crop_v(
    ...
    #height, width = self.get_size(clip)
    #print('start')
    all_constraint_coords = [bbox_from_mask(reference_mask)]
    height, width = img.shape[0], img.shape[1]
    area = height * width
    #ratio = self.ratio
    #scale = self.scale
