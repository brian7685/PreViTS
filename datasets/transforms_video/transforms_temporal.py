...
class RandomStrideCrop: #here
    def __init__(self, size: int, strides=({'stride': 1, 'weight': 1},)):
        self.size = size
        self.strides = strides

        weight_sum = sum(s['weight'] for s in strides) #{'stride': 1, 'weight': 1}
        prefix_sum = 0
    ...
    def __call__(self, frame_indices: np.ndarray):
        ...
        rand = random.random()
        #print('rand',rand) #rand 0.8090313325172959
        idx = bisect_left(self.prefix_weight_sum, rand)
        #print('self.prefix_weight_sum',self.prefix_weight_sum) #self.prefix_weight_sum [1.0]
        #print('idx',idx) #idx 0
        stride = self.strides[idx]['stride']
        #print('stride',stride) #stride 1

        selected = fallback_select(self.size, stride, num_frames)
        #print('selected',selected) #selected None
        if selected is None: #here
            needed_frames = calc_needed_frames(self.size, stride) #32
