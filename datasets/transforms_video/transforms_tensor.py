...
class SequentialGPUCollateFn: #add mask
...
class SequentialGPUCollateFn_visual: #add mask
    ...
    def __call__(self, batch):
        clips,label,class_name,vid,clip_frame_indices_list, *others = zip(*batch)  # [[video * 2], label] -> [[video * 2]], [label]
        #print('clips',clips.shape)
        #print('mask',mask.shape)
        #print('label',label)
        #print('other',other)
        ...
        return (clip_list,label_tensor, class_name, vid,clip_frame_indices_list,*others)


class SequentialGPUCollateFn_mask: #add mask
    ...
    def __call__(self, batch):
        clips, masks, load_mask, info,label, *others = zip(*batch)  # [[video * 2], label] -> [[video * 2]], [label]
        #print('clips',clips.shape)
        #print('mask',mask.shape)
        #print('label',label)
        #print('other',other)
        ...
        for batch_index, clip in enumerate(clips):  # batch of clip0, clip1
            for clip_index, clip_tensor in enumerate(clip):
                clip_tensor = clip_tensor.cuda(device=self.device, non_blocking=True)
                #clip_tensor = ToTensor(clip_tensor)
                clip_tensor = self.transform(clip_tensor)
                """
                try:
        ...
        return (clip_list, mask_list, load_mask, info,label_tensor, *others)

class SequentialGPUCollateFn_mask2: #add mask
...
class SequentialGPUCollateFn_mask2_inter: #add mask
    ...
    def __call__(self, batch):
        ...
        for batch_index, clip in enumerate(clips):  # batch of clip0, clip1
            ...
            for clip_index, clip_tensor in enumerate(clip):
                ...
                if clip_index==0:
                    output_mask_size = 7
                    #mask_tensor_t = mask_tensor[:,:,0,:,:]
                    mask_tensor = torch.nn.functional.interpolate(mask_tensor.type(torch.FloatTensor), \
                    output_mask_size, mode='bilinear', align_corners=True)#.squeeze(0)
                    #print('mask_tensor',mask_tensor.shape) #1177
                mask_tensor = mask_tensor.cuda(device=self.device, non_blocking=True)
                #transforms_spatial.RandomHorizontalFlip()
                random.seed(seed) # apply this seed to img tranfsorms
                np.random.seed(seed)
...
class SequentialGPUCollateFn_mask2_light: #add mask
    ...
    def __call__(self, batch):
        ...
        batch_size = len(clips) #16
        #print('batch_size',batch_size)
        num_clips = len(clips[0]) #2
        clip_list: List[List[Optional[Tensor]]] = [[None for _ in range(batch_size)] for _ in range(num_clips)]
        ...
        for batch_index, clip in enumerate(clips):  # batch of clip0, clip1
            ...
            for clip_index, clip_tensor in enumerate(clip):
                ...
                #torch.set_rng_state(state)
                #mask_tensor = mask
                #print('clip shape',clip_tensor.shape)
                #count non zero
                #print('non zero tensor loading',class_name,vid,np.count_nonzero(masks[batch_index][clip_index]))
                mask_tensor = torch.from_numpy(masks[batch_index][clip_index])
                mask_tensor = mask_tensor.cuda(device=self.device, non_blocking=True)
                ...
                mask_tensor = transform_m(mask_tensor)#self.transform(mask_tensor)
                #print('mask shape',clip_tensor.shape)
                mask_list[clip_index][batch_index] = mask_tensor
                #print('non zero tensor loading',class_name,vid,torch.count_nonzero(mask_tensor))
                """
                try:
...
class SequentialGPUCollateFn_mask2_fix: #add mask
    ...
    def __call__(self, batch):
        ...
        for batch_index, clip in enumerate(clips):  # batch of clip0, clip1
            ...
            for clip_index, clip_tensor in enumerate(clip):
                ...
                #state = torch.get_rng_state()
                #print('state',state,batch_index,clip_index)
                seed = 0#np.random.randint(2147483647)
...
class SequentialGPUCollateFn_mask2_fix_noaug: #add mask
